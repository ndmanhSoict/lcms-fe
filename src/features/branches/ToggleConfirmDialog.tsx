...
export const ToggleConfirmDialog = ({ branch, onClose, onSuccess, onError }: Props) => {
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2.5, textAlign: 'center' }}>
                    <Box
                        sx={{
                            ...
                            backgroundColor: (theme) =>
                                alpha(
                                    isDeactivating ? theme.palette.error.main : theme.palette.success.main,
                                    0.10,
                                ),
                    ...
                    <Box>
                        ...
                        <Typography variant="body1" color="text.secondary">
                            {isDeactivating ? (
                                <>
                                    Bạn có chắc muốn vô hiệu hóa cơ sở{' '}
                                    <strong>{branch?.name}</strong>?{' '}
                                    Cơ sở sẽ tạm thời ngừng hoạt động trong hệ thống.
                                </>
                            ) : (
                                <>
                                    Bạn có chắc muốn kích hoạt lại cơ sở{' '}
                                    <strong>{branch?.name}</strong>?
                                </>
                            )}
                        </Typography>
