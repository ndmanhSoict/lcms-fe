import { styled, Box, Paper, ButtonBase, Avatar, alpha, Chip } from '@mui/material';

const ROW_GRID = 'minmax(200px, 2fr) minmax(150px, 1.5fr) minmax(130px, 1fr) minmax(90px, 0.8fr) minmax(100px, 0.8fr) 100px';
...
export const MetricIconBox = styled(Box)<{ colorScheme: 'primary' | 'success' | 'error' }>(
    ({ theme, colorScheme }) => ({
        width: 56,
        height: 56,
...
export const ClassRow = styled(Paper)(({ theme }) => ({
    display: 'grid',
...
export const ClassAvatar = styled(Avatar)(({ theme }) => ({
    background: `linear-gradient(135deg, ${theme.palette.tertiary?.main ?? theme.palette.primary.main} 0%, ${theme.palette.primary.main} 100%)`,
    color: '#FFFFFF',
    fontWeight: 700,
    width: 44,
    height: 44,
